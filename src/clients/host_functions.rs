...
use ibc::clients::ics10_grandpa::error::Error as ICS10Error;
...
impl HostFunctionsProvider for HostFunctions {
    ...
    fn verify_membership_trie_proof(root: &[u8; 32], proof: &[Vec<u8>], key: &[u8], value: &[u8]) -> Result<(), ICS02Error> {
        let root = H256::from_slice(root);
        let proof_value = read_proof_check::<BlakeTwo256>(root,StorageProof::new(proof.to_vec()), key)
    ...
    fn verify_non_membership_trie_proof(root: &[u8; 32], proof: &[Vec<u8>], key: &[u8]) -> Result<(), ICS02Error> {
        let root = H256::from_slice(root);
        let proof_value = read_proof_check::<BlakeTwo256>(root,StorageProof::new(proof.to_vec()), key)
            .map_err(|_| ICS02Error::read_proof_check())?;
